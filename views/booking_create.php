...
if ($user->getUserType() == "Guest") {
        ...
        <form class="needs-validation card-text" id="createForm" novalidate>
            ...
            <div class="form-group">
                ...
                <input class="form-control" type="tel" id="contactNumber"
                    pattern='^(\+?\d{2})?([\s]?[\-]?[\d]+){8,15}([\s])*' required />
                <div class="invalid-feedback">
    ...
    <script>
        ...
        if (isEditing()) {
            ...
            echo '
        $(function () {
            $("#checkinDate").datepicker("option",{ minDate: 0, dateFormat: "dd/mm/yy"});
            $("#checkoutDate").datepicker("option",{ minDate: 1, dateFormat: "dd/mm/yy"});
        });
        ';
        }
        ?>
        $('#checkinDate').datepicker()
            .on("input change", event => {
                if ($('#checkinDate').datepicker("getDate") != null) {
                    let inDate = $('#checkinDate').datepicker("getDate");
                    inDate.setDate(inDate.getDate() + 1)
                    $(function () {
                        $("#checkoutDate").datepicker("option", { minDate: inDate, dateFormat: "dd/mm/yy" });
                    });
                } else {
                    $(function () {
                        $("#checkoutDate").datepicker("option", { minDate: 1, dateFormat: "dd/mm/yy" });
